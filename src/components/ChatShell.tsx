...
/**
 * UI de chat reutilizable (lista de mensajes + input), extraída del Chat de
 * prueba. El endpoint recibe { messages } y devuelve { reply }. El historial
 * vive solo en memoria de la sesión.
 */
export function ChatShell({
  ...
  placeholder: string;
  /** Render personalizado del contenido de los mensajes del asistente. */
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-3">
        {messages.length === 0 && (
          <div className="h-full grid place-items-center text-center">
            <div className="text-violet-300/50 text-sm max-w-xs">{emptyState}</div>
          </div>
        )}
        {messages.map((m, i) => (
