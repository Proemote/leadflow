...
- **Borradores:** el modelo presenta los seguimientos entre `[BORRADOR]…[/BORRADOR]`; la UI los renderiza como bloque destacado con botón "Copiar" (clipboard). Nunca se envían automáticamente.
- **UI de chat extraída a `ChatShell.tsx`** (lista de mensajes + input, antes dentro de `TestChat.tsx`); `TestChat` y el nuevo `AssistantChat.tsx` son wrappers finos sobre ella. Historial solo en memoria de sesión (sin persistencia, a propósito).
- **Pestaña "Asistente interno"** en Leo (`LeoTabs` pasa a 3 tabs): editor de personalidad propio (`InternalAssistantForm.tsx`) guardado en la clave `internal_assistant_prompt` de `settings` — separada del `system_prompt` de WhatsApp, que no se toca. `/api/settings` acepta ahora `{system_prompt}` o `{internal_prompt}`. Reglas fijas de solo lectura añadidas en código por encima del prompt editable (mismo patrón que las reglas irrompibles de WhatsApp). Nota: `settings` sigue sin `user_id`, así que este prompt es global como el de WhatsApp (limitación conocida).
- **Accesos:** entrada "Hablar con Leo" en sidebar y MobileSidebar (entre "Leo" y "Chat de prueba") + botón secundario "💬 Hablar con Leo" junto a "Empezar mi jornada" en la card de briefing del Panel.
- Verificado: `tsc --noEmit` limpio + `next build` OK (copia en /tmp por el bloqueo TCC) + test end-to-end real con la cuenta de Carlos: consulta de leads/agenda con datos reales y borrador para Ana generado con contexto correcto de su ficha ("Lo Quiero Limpio"), envuelto en los marcadores.

### v1.8 — "Panel con jerarquía + /jornada" (19 julio, commit `10c5a5d`)
